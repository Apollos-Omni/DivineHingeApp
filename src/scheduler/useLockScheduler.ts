...
function nowLocalHHMM(d = new Date()) {
  return (
...
export function useLockScheduler() {
  ...
  useEffect(() => {
    const tick = () => {
      const cur = new Date();
      const hhmm = nowLocalHHMM(cur);
      schedules.forEach((sc) => {
        if (!sc.enabled) return;
        if (sc.kind === "daily" && sc.time === hhmm) {
          setLock(sc.doorId, sc.action === "lock");
          add({
            type: "schedule",
            doorId: sc.doorId,
            ts: Date.now(),
        ...
        if (sc.kind === "once") {
          ...
          if (Math.abs(t.getTime() - cur.getTime()) < 30 * 1000) {
            setLock(sc.doorId, sc.action === "lock");
            add({
              type: "schedule",
              doorId: sc.doorId,
