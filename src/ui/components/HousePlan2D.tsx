...
type Props = {
  ...
  onPixelPositionChange?: (pos: { x:number; y:number; r:number; width:number; height:number }) => void;
};

export default function HousePlan2D({ layout, initialPos = {x:0.12, y:0.12}, onDoorTap, allowTapToMove = true, onPixelPositionChange }: Props) {
  ...
  const onTouch = useCallback((nx: number, ny: number) => {
    ...
    targetRef.current.x = nx; targetRef.current.y = ny; targetRef.current.active = true;
  }, [allowTapToMove]);
  ...
  return (
    <View style={{ flex:1 }} onLayout={onLayout}>
      ...
      {/* default character dot (you can overlay a sprite in parent using onPixelPositionChange) */}
      <Animated.View style={[
        {
          position:"absolute",
          width: N2X(R*2), height: N2Y(R*2), borderRadius: 999,
