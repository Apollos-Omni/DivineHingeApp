...
export default function HousePlan2D({
  layout,
  initialPos = { x: 0.12, y: 0.12 },
  onDoorTap,
  allowTapToMove = true,
  ...
  useFrameCallback((frame) => {
    ...
    if (!targetRef.current.active) {
      ...
      if (s > 0) {
        const d = Math.min(FRICTION * dt, s);
        vx.value *= (s - d) / s;
        vy.value *= (s - d) / s;
      }
    }
  ...
  const playerStyle = useAnimatedStyle(() => ({
    transform: [
      { translateX: N2X(px.value - R) },
      { translateY: N2Y(py.value - R) },
    ],
  }));

  const doorHit = (door: Door, lx: number, ly: number) => {
    const dx = Math.abs(lx - door.center.x),
      dy = Math.abs(ly - door.center.y);
    ...
    <View style={{ flex: 1 }} onLayout={onLayout}>
      ...
      {allowTapToMove && (
        <View
          ...
          onResponderGrant={(e) =>
            onTouch(
              e.nativeEvent.locationX / size.w,
              e.nativeEvent.locationY / size.h,
            )
          }
          onResponderMove={(e) =>
            onTouch(
