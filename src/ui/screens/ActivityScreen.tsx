      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            {item.doorId ? (
              <Text style={styles.sub}>Door: {item.doorId}</Text>
            ) : null}
            {item.note ? <Text style={styles.sub}>{item.note}</Text> : null}
          </View>
        )}
      />
    </View>
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: "#1a1624", borderRadius: 12, padding: 12 },
