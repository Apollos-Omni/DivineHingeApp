...
const DoorCard = ({
  ...
  image,
  onOpen,
}: { title: string; image: any; onOpen: () => void }) => {
  const scale = useSharedValue(1);
  const openAnim = useSharedValue(0);

  const onPress = () => {
    scale.value = withTiming(1.05, { duration: 140, easing: Easing.out(Easing.quad) }, () => {
      openAnim.value = withTiming(1, { duration: 500, easing: Easing.inOut(Easing.cubic) }, () => runOnJS(onOpen)());
    });
  };

  const rStyle = useAnimatedStyle(() => ({ transform: [{ scale: scale.value }] }));
  const doorStyle = useAnimatedStyle(() => ({
    transform: [{ perspective: 600 }, { rotateY: `${-openAnim.value * 70}deg` }],
  }));
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: '#0b0b0f' }} contentContainerStyle={{ padding: 16 }}>
      ...
      <View style={{ gap: 14 }}>
        ...
        <DoorCard title="Locks" image={require('../../assets/backgrounds/door1.avif')} onOpen={() => navigation.navigate('Locks')} />
        <DoorCard title="Schedules" image={require('../../assets/backgrounds/door2.avif')} onOpen={() => navigation.navigate('Schedules')} />
        <DoorCard title="Video" image={require('../../assets/backgrounds/door3.avif')} onOpen={() => navigation.navigate('Video')} />
      </View>
    </ScrollView>
  );
}
